...
class Sukima_ModelProvider(ModelProvider):
    ...
    async def generate_async(self, args: ModelGenRequest):
        ...
        async with aiohttp.ClientSession() as session:
            try:
                async with session.post(f'{self.endpoint_url}/api/v1/models/generate', json=json.dumps(args.__dict__, indent=4, sort_keys= true, default= str), headers={'Authorization': f'Bearer {self.token}'}) as resp:
                    if resp.status == 200:
                        js = await resp.json()
